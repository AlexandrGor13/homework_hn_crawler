...
from crawler.settings import base_url, request_timeout, max_retries, max_concurrent_requests
...
class PageFetcher:
    ...
    async def fetch(self, url: str, retries: int = max_retries) -> FetchResult:
        """Опрашивает страницу с повторениями"""
        if not self.session:
            await self.start()
    ...
    async def fetch_hn_page(self, page: str = "") -> str:
        """Извлекает страницу Hacker News"""
        url = f"{base_url}/{page}"
